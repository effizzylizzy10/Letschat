...
/* ============================================================
   LOOP — realtime chat client
   Talks to the Loop server (Express + Socket.io) configured in config.js.
...
const { API_URL, SOCKET_URL } = window.LOOP_CONFIG;
...
function loadJSON(key, fallback) {
  try {
    const raw = window.localStorage.getItem("loop:" + key);
    return raw === null ? fallback : JSON.parse(raw);
  } catch { return fallback; }
}
function saveJSON(key, value) {
  try { window.localStorage.setItem("loop:" + key, JSON.stringify(value)); }
  catch (e) { console.error("Storage save failed", e); }
}
function clearJSON(key) {
  try { window.localStorage.removeItem("loop:" + key); } catch {}
}
    ...
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ fontFamily: "Inter", fontSize: 13, color: "#8891A0", marginBottom: 16 }}>
          Enter the Loop phone number of the person you want to message. They need to have signed in to Loop at least once.
        </div>
...
function ChatsScreen({ token, profile, conversations, loading, error, onOpenChat, onProfile, onNewChat, presence }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <TopBar
        title={<span>lo<span style={{ color: "#35D0BA" }}>o</span>p</span>}
        right={
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 28px", background: "radial-gradient(circle at 50% 0%, #12251F 0%, #0E1116 62%)" }}>
      <div style={{ textAlign: "center", marginBottom: 36 }}>
        <div style={{ width: 76, height: 76, borderRadius: 22, margin: "0 auto 20px", background: "conic-gradient(from 120deg, #35D0BA, #F2B84B, #35D0BA)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ width: 66, height: 66, borderRadius: 18, background: "#0E1116", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontFamily: "Sora", fontWeight: 800, fontSize: 26, color: "#35D0BA" }}>lo<span style={{ color: "#F2B84B" }}>o</span>p</span>
          </div>
        </div>
        ...
        <p style={{ fontFamily: "Inter", fontSize: 14, color: "#8891A0", margin: 0, lineHeight: 1.5 }}>
          {needsName ? "This is what people you chat with will see." : "This becomes your Loop ID — share it so others can message you."}
        </p>
      </div>
      ...
      <p style={{ textAlign: "center", fontFamily: "Inter", fontSize: 12, color: "#5B6673", marginTop: 22, lineHeight: 1.6 }}>
        Loop connects two people over the internet in realtime — <br />no phone verification code is actually sent yet.
      </p>
    </div>
  );
}
...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on("connect_error", (err) => setConvError("Can't reach the Loop server: " + err.message));
